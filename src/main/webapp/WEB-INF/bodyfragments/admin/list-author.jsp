...
<div class="main-content item">
    ...
    <!--Start list authors -->
    <div class="list-author-container flex flex-col items-center gap-2">
        <h3>${message }</h3>

        <table class="w-full text-sm text-left text-gray-500">
            <thead class="text-xs text-gray-700 uppercase bg-gray-100">
            <tr>
                <th class="px-6 py-3 border-0 rounded-l-lg"><a href="#"><input
                        type="checkbox" name="checkbox-delete-btn" id="select-all-row"/></a></th>
                <c:choose>
                    <c:when test="${sortName == 'id' && sortType == 'DESC'}">
                        <th class="px-6 py-3 border-0"><a
            ...
            <c:forEach items="${pageObj.content}" var="author">
                ...
                <tr class="bg-white hover:bg-gray-50 transition-all [&:not(:last-child)]:border-b">
                    <td class="px-6 py-4 border-0 rounded-l-lg"><input class="select-row"
                                                                       type="checkbox" name="checkbok-delete"
                                                                       value="${author.id}"/></td>
                    <td class="px-6 py-4 border-0">${author.id}</td>
                    <td class="px-6 py-4 border-0 tr-width-12">${author.fullname}</td>
                    <td class="px-6 py-4 border-0 tr-width-12"><fmt:formatDate
                            value="${author.birthday}" pattern="dd/MM/yyyy"/></td>
                    <td class="px-6 py-4 border-0 tr-width-12">${author.address}</td>
                    <td class="px-6 py-4 border-0 tr-width-12">${author.phone}</td>
                    <td class="px-6 py-4 border-0 tr-width-12">${author.status == 1 ? "Kích hoạt" : "Không kích hoạt"}</td>
                    <td class="px-6 py-4 border-0 tr-width-12"><fmt:formatDate
                    ...
                    <td class="px-6 py-4 border-0 rounded-r-lg">
                        <div class="custom-btn">
                            <a href="/admin/authors/update/${author.id}" class="edit-custom-btn">Sửa</a> <a href="#"
                                                                                                            ...
                                                                                                            onclick="deleteSelected([${author.id}])">Xóa</a>
                        </div>
                    </td>
                </tr>
            </c:forEach>
            ...
            <nav class="m-4">
                <ul class="flex items-center -space-x-px h-8 gap-1 text-sm">
                    <c:if test="${not pageObj.first}">
                        <li>
                            <a class="page-link flex rounded rounded-l-lg items-center justify-center px-3 h-8 ml-0 leading-tight text-gray-500 bg-white border border-gray-300 hover:bg-gray-100 hover:text-gray-700"
                    ...
                    <c:if test="${pageObj.number > 0}">
                        <li class="page-item"><a
                                class="page-link flex rounded items-center justify-center px-3 h-8 ml-0 leading-tight text-gray-500 bg-white border border-gray-300 hover:bg-gray-100 hover:text-gray-700"
                                href="/admin/authors?search=${search}&page=${pageObj.number}">${pageObj.number}</a>
                        </li>
                    </c:if>
                    <li class="page-item"><a
                            class="page-link flex rounded items-center justify-center px-3 h-8 ml-0 leading-tight text-gray-500 bg-white border border-gray-300 hover:bg-gray-100 hover:text-gray-700 active"
                            href="/admin/authors?search=${search}&page=${pageObj.number + 1}">${pageObj.number + 1}</a>
                    </li>
                    <c:if test="${pageObj.number + 1 < pageObj.totalPages}">
                        <li class="page-item"><a
                                class="page-link flex rounded items-center justify-center px-3 h-8 ml-0 leading-tight text-gray-500 bg-white border border-gray-300 hover:bg-gray-100 hover:text-gray-700"
                                href="/admin/authors?search=${search}&page=${pageObj.number + 2}">${pageObj.number + 2}</a>
                        </li>
                    </c:if>
                    <c:if test="${not pageObj.last}">
                        <li class="page-item"><a
                                class="page-link flex rounded rounded-r-lg items-center justify-center px-3 h-8 ml-0 leading-tight text-gray-500 bg-white border border-gray-300 hover:bg-gray-100 hover:text-gray-700"
